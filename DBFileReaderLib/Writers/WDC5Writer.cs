...
{
    class WDC5RowSerializer<T> : IDBRowSerializer<T> where T : class
    {
        ...
        public WDC5RowSerializer(BaseWriter<T> writer)
        {
            m_writer = writer;
    ...
    class WDC5Writer<T> : BaseWriter<T> where T : class
    {
        ...
        public WDC5Writer(WDC5Reader reader, IDictionary<int, T> storage, Stream stream) : base(reader)
        {
            ...
            WDC5RowSerializer<T> serializer = new WDC5RowSerializer<T>(this);
            ...
            using (var writer = new BinaryWriter(stream))
            {
                ...
                // section header
                int fileOffset = HeaderSize + (Meta.Length * 4) + (ColumnMeta.Length * 24) + Unsafe.SizeOf<SectionHeaderWDC5>() + palletDataSize + commonDataSize;
